...
internal class PaletteWidget : IWidgetItem, IWidgetUpdateDraw
{
    ...
    public void Draw(IWidgetLog logger)
    {
        ...
        var size = new ImVec2(width * coloursPerRow, height * rows);
        var remainder = availColours == coloursPerRow ? availColours : availColours % coloursPerRow;
        if (ImGui.BeginChild($"palette", size, 0, 0))
        {
            ...
            for (int y = 0; y < rows; y++)
            {
                ...
                for (int x = 0; x < numColours; x++)
                {
                    ...
                    drawList.AddRectFilled(new ImVec2(pos.X + xOffs + 4, pos.Y + yOffs + 4), new ImVec2(pos.X + xOffs + width - 4, pos.Y + yOffs + height - 4), ImGuiHelper.MakeColour(colour.Red, colour.Green, colour.Blue, colour.Alpha));
                    if (_iPalette.SelectedColour == a)
                    {
                        drawList.AddRect(new ImVec2(pos.X + xOffs, pos.Y + yOffs), new ImVec2(pos.X + xOffs + width, pos.Y + yOffs + height), ImGuiHelper.MakeColour(255, 255, 255, 255));
                    }
                    a++;
                    xOffs += width;
                }
