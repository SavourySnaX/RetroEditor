...
internal class Megadrive68000Disassembler : DisassemblerBase
{
    ...
    public override List<MemoryAccess> FetchMappedAccesses(Instruction ins, ICpuRegisterState registerState, IMemoryMapper memoryMapper)
    {
        ...
        foreach (var operand in ins.Operands)
        {
            ...
            if (operand is OM68000_AddressIndirect)
            {
                int regNum = (int)operand.Value;
                effectiveAddress = registers.AddressRegisters[regNum];
                hasAccess = true;
            }
            else if (operand is OM68000_AddressPostInc)
            {
                int regNum = (int)operand.Value;
                effectiveAddress = registers.AddressRegisters[regNum];
                hasAccess = true;
            }
            else if (operand is OM68000_AddressPreDec)
            {
                int regNum = (int)operand.Value;
                effectiveAddress = registers.AddressRegisters[regNum];
                hasAccess = true;
            ...
            else if (operand is OM68000_AddressIndex index)
            {
                ...
                UInt32 indexValue = useAddressReg
                    ? registers.AddressRegisters[indexReg]
                    : registers.DataRegisters[indexReg];
            ...
            else if (operand is OM68000_AbsoluteShort)
            {
                effectiveAddress = (UInt16)operand.Value;
                hasAccess = true;
            }
            else if (operand is OM68000_AbsoluteLong)
            {
